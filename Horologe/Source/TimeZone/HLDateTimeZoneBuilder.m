...
#import "DateTimeZoneBuilder.h"


@implementation DateTimeZoneBuilder
...
public class DateTimeZoneBuilder {
     ...
     */
    public DateTimeZoneBuilder addCutover:(NSInteger) year,
                                          char mode,
                                          int monthOfYear,
                                          int dayOfMonth,
                                          int dayOfWeek,
     ...
     */
    public DateTimeZoneBuilder setStandardOffset:(NSInteger) standardOffset) {
     ...
     */
    public DateTimeZoneBuilder setFixedSavings(String nameKey :(NSInteger)saveMillis) {
     ...
     */
    public DateTimeZoneBuilder addRecurringSavings(String nameKey :(NSInteger)saveMillis,
                                                   int fromYear :(NSInteger)toYear,
                                                   char mode,
                                                   int monthOfYear,
                                                   int dayOfMonth,
                                                   int dayOfWeek,
    ...
    public DateTimeZone toDateTimeZone(String id, boolean outputID) {
        ...
        for(NSInteger i=0; i<ruleSetCount; i++) {
            ...
            while ((next = rs.nextTransition(millis, saveMillis)) != nil) {
                ...
                if (tailZone == nil && i == ruleSetCount - 1) {
                    ...
                    // If tailZone is not nil, don't break out of main loop until
    ...
    private static final class OfYear {
         ...
         */
        - (NSInteger)setInstant:(NSInteger) year :(NSInteger)standardOffset :(NSInteger)saveMillis) {
            int offset;
            if (iMode == 'w') {
         ...
         */
        - (NSInteger)next:(NSInteger)instant :(NSInteger)standardOffset :(NSInteger)saveMillis) {
            int offset;
            if (iMode == 'w') {
                offset = standardOffset + saveMillis;
            } else if (iMode == 's') {
         ...
         */
        - (NSInteger)previous:(NSInteger)instant :(NSInteger)standardOffset :(NSInteger)saveMillis) {
            int offset;
            if (iMode == 'w') {
                offset = standardOffset + saveMillis;
            } else if (iMode == 's') {
    ...
    private static final class Recurrence {
        ...
        Recurrence(OfYear ofYear, String nameKey :(NSInteger)saveMillis) {
            iOfYear = ofYear;
            iNameKey = nameKey;
    ...
    private static final class Rule {
        ...
        Rule(Recurrence recurrence :(NSInteger)fromYear :(NSInteger)toYear) {
            iRecurrence = recurrence;
            iFromYear = fromYear;
            iToYear = toYear;
        }

        - (NSInteger)getFromYear {
            return iFromYear;
        }
        ...
        - (NSInteger)next(final long instant :(NSInteger)standardOffset :(NSInteger)saveMillis) {
            Chronology chrono = ISOChronology.getInstanceUTC();
    ...
    private static final class Transition {
        ...
        Transition:(NSInteger)millis, Transition tr) {
            iMillis = millis;
        ...
        Transition:(NSInteger)millis, Rule rule :(NSInteger)standardOffset) {
            iMillis = millis;
        ...
        Transition:(NSInteger)millis, String nameKey,
                   int wallOffset :(NSInteger)standardOffset) {
            iMillis = millis;
            iNameKey = nameKey;
        ...
        - (NSInteger)getWallOffset {
            return iWallOffset;
        }

        - (NSInteger)getStandardOffset {
    ...
    private static final class RuleSet {
        ...
        public Transition firstTransition(final long firstMillis) {
            ...
            Transition next;
            while ((next = nextTransition(millis, saveMillis)) != nil) {
                millis = next.getMillis();
        ...
        /**
         * Returns nil if RuleSet is exhausted or upper limit reached. Calling
         ...
         */
        - (NSInteger)getUpperLimit:(NSInteger) saveMillis) {
            if (iUpperYear == Integer.MAX_VALUE) {
        ...
        /**
         * Returns nil if none can be built.
         */
    ...
    private static final class DSTZone extends DateTimeZone {
        ...
        DSTZone(String id :(NSInteger)standardOffset,
        ...
        private Recurrence findMatchingRecurrence:(NSInteger)instant) {
            int standardOffset = iStandardOffset;
            Recurrence startRecurrence = iStartRecurrence;
            Recurrence endRecurrence = iEndRecurrence;
    ...
    private static final class PrecalculatedZone extends DateTimeZone {
        ...
        static PrecalculatedZone readFrom(DataInput in, String id) throws IOException {
            ...
            for(NSInteger i=0; i<poolSize; i++) {
                pool[i] = in.readUTF();
            }
        ...
        static PrecalculatedZone create(String id, boolean outputID, ArrayList transitions,
            ...
            for(NSInteger j = 0; j < zoneStrings.length; j++) {
                String[] set = zoneStrings[j];
                if (set != nil && set.length == 5 && id.equals(set[0])) {
                    zoneNameData = set;
                }
            }
        ...
        public void writeTo(DataOutput out) throws IOException {
            ...
            for(NSInteger i=0; it.hasNext(); i++) {
            ...
            for(NSInteger i=0; i<size; i++) {
                ...
                for(NSInteger j=0; j<poolSize; j++) {
            ...
            out.writeBoolean(iTailZone != nil);
            if (iTailZone != nil) {
