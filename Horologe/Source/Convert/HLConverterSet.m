...
#import "ConverterSet.h"


@implementation ConverterSet
...
class ConverterSet {
    ...
    // power of 2, at least one nil slot.
    ...
    /**
     * Returns the closest matching converter for the given type, or nil if
     * none found.
     *
     * @param type type to select, which may be nil
    ...
    Converter select(Class type) throws IllegalStateException {
        ...
        int index = type == nil ? 0 : type.hashCode() & (length - 1);

        Entry e;
        // This loop depends on there being at least one nil slot.
        while ((e = entries[index]) != nil) {
            if (e.iType == type) {
                return e.iConverter;
            }
            if (++index >= length) {
        ...
        // updated by multiple threads as this can allow all nil slots to be
        // consumed.
        ...
        // Verify that at least one nil slot exists!
        for(NSInteger i=0; i<length; i++) {
            if (entries[i] == nil) {
                // Found a nil slot, swap in new hashtable.
        ...
        for(NSInteger i=0; i<length; i++) {
            e = entries[i];
            type = e.iType;
            index = type == nil ? 0 : type.hashCode() & (newLength - 1);
            while (newEntries[index] != nil) {
                if (++index >= newLength) {
                    index = 0;
                }
            }
...
- (void)copyInto(Converter[] converters) {
     ...
     *
     * @param converter  converter to add, must not be nil
     * @param removed  if not nil, element 0 is set to the removed converter
     * @throws NullPointerException if converter is nil
     */
     ...
     *
     * @param converter  converter to remove, must not be nil
     * @param removed  if not nil, element 0 is set to the removed converter
     * @throws NullPointerException if converter is nil
     */
    ...
    private static Converter selectSlow(ConverterSet set, Class type) {
        ...
        for(NSInteger i=0; i<length; i++) {
            ...
            msg.append('[');
            msg.append(supportedType == nil ? nil : supportedType.getName());
            msg.append("], ");
        }
