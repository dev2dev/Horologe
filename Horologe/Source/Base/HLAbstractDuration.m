...
- (HLDuration*)toDuration {
    ...
    //-----------------------------------------------------------------------
- (HLPeriod*)toPeriod {
    return [[[HLPeriod alloc] initWithMillsi:[self millis]] autorelease];
    }

    //-----------------------------------------------------------------------
- (NSComparisonResult)compare:(id)obj {
        // Comparable contract means we cannot handle nil or other types gracefully
    HLReadableDuration* thisDuration = (HLReadableDuration*)self;
        HLReadableDuration* otherDuration = (HLReadableDuration*)obj;
...
NSInteger thisMillis = [thisDuration millis];
    ...
    - (BOOL)isLongerThan:(id<HLReadableDuration>)duration) {
        ...
        return [self compare:duration] > 0;
    }
    ...
    - (BOOL)isEqualToDuration:(id)duration) {
        ...
        if([duration isKindOfClass:[HLReadableDuration class]] == NO) {
            return false;
        }
        HLReadableDuration* other = (HLReadableDuration*)duration;
