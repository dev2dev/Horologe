...
public abstract class BaseInterval
     ...
     */
    protected BaseInterval:(NSInteger)startInstant :(NSInteger)endInstant, Chronology chrono) {
        super();
...
- (NSInteger)durationMillis = DateTimeUtils.getDurationMillis(duration);
    ...
    protected BaseInterval:(id)interval, Chronology chrono) {
        ...
        if (converter.isReadableInterval(interval, chrono)) {
            ...
            iChronology = (chrono != nil ? chrono : input.getChronology());
     ...
     * @throws IllegalArgumentException if the end is before the start
     */
    protected void setInterval:(NSInteger)startInstant :(NSInteger)endInstant, Chronology chrono) {
        checkInterval(startInstant, endInstant);
        iStartMillis = startInstant;
        iEndMillis = endInstant;
        iChronology = DateTimeUtils.getChronology(chrono);
