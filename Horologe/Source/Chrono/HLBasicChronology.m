...
#import "BasicChronology.h"


@implementation BasicChronology
...
abstract class BasicChronology extends AssembledChronology {
    ...
    - (NSInteger)getDateTimeMillis(
            int year :(NSInteger)monthOfYear :(NSInteger)dayOfMonth :(NSInteger)millisOfDay)
    ...
    - (NSInteger)getMinimumDaysInFirstWeek {
     ...
     */
    int getDaysInYear:(NSInteger) year) {
...
- (NSInteger)oneYear;
     ...
     */
    abstract int getMonthOfYear:(NSInteger)millis :(NSInteger)year);
     ...
     * @param year precalculated year of millis
     */
    int getDayOfMonth:(NSInteger)millis :(NSInteger)year) {
        int month = getMonthOfYear(millis, year);
        return getDayOfMonth(millis, year, month);
    }
...
- (NSInteger)dateMillis = getYearMillis(year);
     ...
     * @param instant millis from 1970-01-01T00:00:00Z
     */
    int getDayOfYear:(NSInteger)instant) {
...
- (NSInteger)yearStart = getYearMillis(year);
     ...
     * @param instant millis from 1970-01-01T00:00:00Z
     */
    int getWeekyear:(NSInteger)instant) {
     ...
     * @param instant millis from 1970-01-01T00:00:00Z
     */
    int getWeekOfWeekyear:(NSInteger)instant) {
...
- (NSInteger)daysSince19700101;
     ...
     * @param instant millis from 1970-01-01T00:00:00Z
     */
    int getMillisOfDay:(NSInteger)instant) {
        if (instant >= 0) {
     ...
     */
    int getDaysInMonthMax:(NSInteger)instant) {
     ...
     * @return the maximum number of days in the month
     */
    int getDaysInMonthMaxForSet:(NSInteger)instant :(NSInteger)value) {
...
- (NSInteger)getDateMidnightMillis:(NSInteger) year :(NSInteger)monthOfYear :(NSInteger)dayOfMonth) {
     ...
     */
    abstract long getYearDifference:(NSInteger)minuendInstant :(NSInteger)subtrahendInstant);
     ...
     * @return true if leap
     */
    abstract boolean isLeapYear:(NSInteger) year);
     ...
     */
    abstract int getDaysInYearMonth:(NSInteger) year :(NSInteger)month);
     ...
     * @return the max days
     */
    abstract int getDaysInMonthMax:(NSInteger) month);
     ...
     */
    abstract long getTotalMillisByYearMonth:(NSInteger) year :(NSInteger)month);
     ...
     */
    abstract long calculateFirstDayOfYearMillis:(NSInteger) year);
     ...
     */
    int getMaxMonth:(NSInteger) year) {
        return getMaxMonth();
    }
     ...
     */
    abstract long setYear:(NSInteger)instant :(NSInteger)year);

    //-----------------------------------------------------------------------
    // Although accessed by multiple threads, this method doesn't need to be synchronized.
    private YearInfo getYearInfo:(NSInteger) year) {
        YearInfo info = iYearInfoCache[year & CACHE_MASK];
        if (info == nil || info.iYear != year) {
    ...
    private static class HalfdayField extends PreciseDateTimeField {
        ...
        - (NSInteger)set:(NSInteger)millis, String text locale:(NSLocale*)locale {
    ...
    private static class YearInfo {
        ...
        YearInfo:(NSInteger) year :(NSInteger)firstDayMillis) {
            iYear = year;
