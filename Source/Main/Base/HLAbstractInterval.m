...
#import "Base/HLAbstractInterval.h"
#import "Core/HLDateTimeUtils.h"
#import "Core/HLDateTime.h"
#import "Core/HLMutableInterval.h"
...
@implementation HLAbstractInterval
...
//-----------------------------------------------------------------------
- (void)checkIntervalStart:(NSInteger)start 
                       end:(NSInteger)end {
    if (end < start) {
        [NSException raise:NSInvalidArgumentException format:@"The end instant must be greater or equal to the start"];
    }
}

//-----------------------------------------------------------------------
- (HLDateTime*)start {
    return [[[HLDateTime alloc] initWithMillis:[self startMillis] chronology:[self chronology]] autorelease];
}

- (HLDateTime*)end {
    return [[[HLDateTime alloc] initWithMillis:[self endMillis] chronology:[self chronology]] autorelease];
}
...
- (BOOL)overlaps:(id<HLReadableInterval>)interval {
    long thisStart = [self startMillis];
    long thisEnd = [self endMillis];
...
- (HLInterval*)toInterval {
    return [[[HLInterval alloc] initWithStart:[self startMillis] end:[self endMillis] chronology:[self chronology]] autorelease];
}

- (HLMutableInterval*)toMutableInterval {
    return [[[HLMutableInterval alloc] initWithStart:[self startMillis] end:[self endMillis] chronology:[self chronology]] autorelease];
}

//-----------------------------------------------------------------------
- (NSInteger)toDurationMillis {
    return [HLFieldUtils safeAdd:[self endMillis] to:-([self startMillis])];
}

- (HLDuration*)toDuration {
    NSInteger durMillis = [self toDurationMillis];
    if (durMillis == 0) {
        return HL_DURATION_ZERO;
    } 
    else {
...
//-----------------------------------------------------------------------
- (HLPeriod*)toPeriod {
    return [[[HLPeriod alloc] initWithStart:[self startMillis] end:[self endMillis] chronology:[self chronology]] autorelease];
}

- (HLPeriod*)toPeriod:(HLPeriodType*)type {
    return [[[HLPeriod alloc] initWithStart:[self startMillis] end:[self endMillis] type:type chronology:[self chronology]] autorelease];
}
...
- (BOOL)isEqual:(id)readableInterval {
    ...
    if (![readableInterval conformsToProtocol:@protocol(HLReadableInterval)]) {
        return NO;
    }
    
    id<HLReadableInterval> other = (id<HLReadableInterval>) readableInterval;
    
    return [self startMillis] == [other startMillis] && [self endMillis] == [other endMillis] && [HLFieldUtils isObject:[self chronology] equalTo:[other chronology]];
}
...
- (NSInteger)hashCode {
    ...
    result = 31 * result + [[self chronology] hashCode];
    return result;
}
...
- (NSString*)description {
    ...
    NSMutableString* buf = [[NSMutableString alloc] initWithCapacity:48];
    [printer printTo:buf millis:[self startMillis]];
    [buf appendString:@"/"];
    [printer printTo:buf millis:[self endMillis]];
